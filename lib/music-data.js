...
function getArtistFraming(artist, jeffRole) {
  if (artist.toLowerCase().includes("jeff berlin")) {
    return "It sits inside Jeff Berlin's own public catalog rather than the sideman margins.";
  }

  if (jeffRole.toLowerCase().includes("track")) {
    return `It shows how Jeff's playing circulated beyond leader dates through curated releases credited to ${artist}.`;
  }

  return `It places Jeff inside ${artist}'s orbit, which is exactly what makes this part of the archive valuable.`;
}
...
function getSpecificHook(title, artist, kind, jeffRole) {
  switch (kind) {
    case "instructional":
      return `${title} is not just another entry in the discography. It documents Jeff Berlin presenting his ideas directly, which makes it part performance artifact and part teaching statement.`;
    case "soundtrack":
      return `${title} pushes Jeff Berlin's discography into a broader cultural lane because the credit lives inside a soundtrack context instead of a conventional fusion or jazz release.`;
    case "box-set":
      return `${title} matters because box sets reshape how a career is remembered. They gather scattered periods into one package and turn chronology into a narrative.`;
    case "best-of":
      return `${title} is useful because a best-of collection tells you what material a catalog, label, or curator considered worth carrying forward.`;
    case "live":
      return `${title} matters because live releases change the angle. They show Jeff in a performance environment rather than a purely studio-framed document.`;
    case "sampler":
      return `${title} is a smaller-format archival clue, but it still shows where Jeff's name and playing surfaced when labels or editors wanted a scene snapshot.`;
    case "compilation":
      return `${title} works as an anthology-style checkpoint. Even when Jeff appears as one part of a larger sequence, the release still marks where his work was being recirculated.`;
    case "reissue":
      return `${title} is useful because reissue packages tell you a record continued to matter enough to be repackaged and placed back in circulation.`;
    case "demo":
      return `${title} has archival value because demo-era material reveals process, not just finished product. That gives the page a different kind of interest from a standard album entry.`;
    default:
      if (artist.toLowerCase().includes("jeff berlin")) {
        return `${title} belongs here as a direct Jeff Berlin release, and it helps fill in the parts of his own catalog that tend to get skipped in shorter career summaries.`;
      }

      if (jeffRole.toLowerCase().includes("track")) {
        return `${title} earns its place by showing that Jeff's recorded footprint extends into curated releases where a single track still carried his name into a different audience.`;
      }

      return `${title} belongs in the archive because it is a concrete, dated example of Jeff Berlin showing up in another serious artist's project.`;
  }
}
  ...
  return {
    ...
    snapshot: [
      ...
      coverSource
        ...
        : `This ${formatLabel} sits in the archive as part of Jeff Berlin's broader recorded timeline.`
    ],
    caseStudy: [
      noteLine,
      artistLine,
      `Placed in ${year}, ${title} helps make the discography read less like a highlight reel and more like an actual working timeline.`
    ],
    highlights: [
      {
        title: "Release frame",
        body: `${title} is presented here as a ${formatLabel}, which changes how the credit should be read.`
      },
      {
        title: "Jeff's role",
        body: `On this release Jeff Berlin is documented as ${roleLabel}, giving the page a clear reason to be in the archive.`
      },
      {
        title: "Archive value",
        body: `${title} adds specificity to the larger timeline by pinning Jeff to a real ${year} release instead of a vague credit list.`
      }
    ],
    trackMoments: [
      {
      ...
      {
        title: "Place it in the timeline",
        body: `${title} makes more sense when heard against the Jeff Berlin releases immediately around ${year}, because that is where its real discographic weight shows up.`
      }
    ]
  };
}
...
const featuredMusicAlbums = [
  ...
  },
  {
    ...
    caseStudy: [
      "The reason this record belongs in the archive is that it captures the more aggressive side of Jeff's solo identity. It is still fusion, but it feels punchier and more audience-facing than some of the earlier ensemble records associated with his name.",
  ...
  },
  {
    ...
    caseStudy: [
      "This album matters because it gives the solo catalog a second chapter. It is not the 1980s Jeff Berlin trying to announce himself anymore. It is a more seasoned Jeff shaping a record with the authority of somebody who already knows the stakes.",
      ...
      "On this site, it works as the pivot point between the early-fusion-solo era and the later records where Jeff's identity becomes even more self-defined."
    ],
    highlights: [
      {
        title: "A solo return",
  ...
  },
  {
    ...
    caseStudy: [
      "This record earns a place here because it hints at Jeff's contrarian streak. Even the title pushes back on the idea that jazz has to present itself as refined wallpaper.",
      ...
      "Lumpy Jazz also helps the archive because it breaks the timeline open. It shows Jeff still naming, framing, and shaping albums in a very personal way decades after the first solo records."
    ],
    highlights: [
      {
  ...
  },
  {
    ...
    caseStudy: [
      "This album is worth calling out because it opens a little discographic rabbit hole. MusicBrainz treats Ace of Bass and Aneurythms as the same release group, which makes the project interesting beyond the music itself.",
      "That matters for this archive because it shows Jeff's solo catalog has alternate-release history, not just a neat row of isolated records.",
  ...
  },
  {
    ...
    caseStudy: [
      "This page exists separately from Ace of Bass because the title, presentation, and market framing change the way the record reads in the catalog, even if the release group overlaps.",
      ...
      "That difference matters on a site like this. Jeff Berlin's catalog is not just about notes and chops; it is also about how he frames the work and what kind of identity each release projects."
    ],
    highlights: [
      {
  ...
  },
  {
    ...
    caseStudy: [
      ...
      "The record matters because it proves Jeff did not need to hide behind fusion density to sound like himself. Standards material simply shifts the frame; it does not erase the personality.",
      "It also sets up one of the site's best album pairings with Low Standards, which makes the archive feel less like a list and more like a conversation across releases."
    ],
    highlights: [
      {
  ...
  },
  {
    ...
    intro:
      "Low Standards is the kind of title that immediately improves the archive. It reads like a sequel, a joke, a critique, and a mission statement all at once.",
    snapshot: [
    ...
    caseStudy: [
      ...
      "For this site, that makes the album ideal. The name sounds like a provocation, but the deeper point is still seriousness about music."
    ],
    highlights: [
      {
  ...
  },
  {
    ...
    caseStudy: [
      "This album belongs in the archive because it tells you who still matters to Jeff Berlin after decades of playing, teaching, and recording. He is not just honoring a famous bassist. He is engaging with a songwriter and vocalist whose music shaped his own.",
  ...
  },
  {
    ...
    caseStudy: [
      "The record matters because it is not just an early Jeff Berlin credit. It is the sound of several strong musical personalities colliding in a very specific late-1970s progressive-fusion zone and somehow making it feel composed rather than chaotic.",
  ...
  },
  {
    ...
    caseStudy: [
      "This record is a strong case for Berlin as a compositional force inside advanced fusion, not only a virtuoso instrumentalist. His writing credit on 'Five G' matters because it shows how quickly he became central to the band's identity.",
      ...
      "For this site, it earns a place because it shows Jeff Berlin inside an ensemble that was evolving fast and trusting him with more than bass chair duties."
    ],
    highlights: [
      {
  ...
  },
  {
    ...
    caseStudy: [
      "The album leans further toward progressive rock than the earlier Bruford records, and that shift matters because Berlin adapts without losing his precision or identity.",
      "The vocal role is part of what makes this album worth featuring. It catches him in a wider frame and makes the record feel less like a strict fusion statement and more like a band stretching its boundaries.",
  ...
  },
  {
    ...
    intro:
      "This is not a Jeff Berlin headline album, which is exactly why it belongs here. The Story of I shows him inside a much more concept-heavy, globally inflected setting where the bass has to support a larger architecture rather than dominate the room.",
    snapshot: [
    ...
    caseStudy: [
      ...
      "It is also just a cool record for this site because the concept is strange, the personnel is strong, and the overall ambition feels fearless in exactly the way this project should highlight."
    ],
    highlights: [
      {
  ...
  },
  {
    ...
    caseStudy: [
      ...
      "It also adds a useful angle to Jeff Berlin's profile on this site because it shows his name attached to a release that crossed into a more broadly visible awards conversation."
    ],
    highlights: [
      {
...
const discographyAlbums = [
  ...
  makeDiscographyEntry({
    ...
    year: "1998",
    note:
      "A missing Jeff Berlin solo title that belongs in the archive between the late-1990s return and the later independent records.",
    sources: [
  ...
  makeDiscographyEntry({
    ...
    jeffRole: "Instructor",
    note:
      "An official instructional release that belongs in the public Jeff Berlin archive, not just the listening discography.",
    sources: [
